...
import { addListMovie, getMyList } from "../utils/api";

function AddMyList({ onClose }) {
  const { id } = useParams();
  const [tags, setTags] = useState("");

  const navigate = useNavigate();

  const fetchPostMyList = async () => {
    try {
      const token = localStorage.getItem("token");
      await addListMovie(tags, token);
    } catch (error) {
      console.log(error);
    }
  };
  ...
  useEffect(() => {
    const fetchJobDetail = async () => {
      try {
        const token = localStorage.getItem("token");
        const response = await getMyList(token, id);
