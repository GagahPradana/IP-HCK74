...
import { addListNameMovie } from "../utils/api";

function AddMyListName({ onClose }) {
  const [listName, setListName] = useState("");

  const navigate = useNavigate();

  const fetchPostListName = async () => {
    try {
      const token = localStorage.getItem("token");
      await addListNameMovie(listName, token);
      navigate("/mylist");
    } catch (error) {
      console.log(error);
    }
